...
namespace JHS
{
    public class HealBeamStatic : MonoBehaviour
    {
        ...
        void FixedUpdate()
        {
            if (beam) //Updates the beam
            {
                ...
                Vector3 end = BossSystem.Instance.BossTr.position + endOffset;
        ...
        public void SpawnBeam() //This function spawns the prefab with linerenderer
        {
            if (beamLineRendererPrefab)
            {
                if (beamStartPrefab)
                {
                    ...
                    beamStart.transform.localScale = Vector3.one;
                }
                if (beamEndPrefab)
                {
                    beamEnd = Instantiate(beamEndPrefab);
                    beamEnd.transform.parent = transform;
                    beamEnd.transform.localScale = Vector3.one;
                }
